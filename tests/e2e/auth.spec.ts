...
test.describe('Autenticação', () => {
  test('deve exibir a página de login', async ({ page }) => {
    ...
    const heading = page.locator('h2:has-text("Entre na sua conta")');
  ...
  test('deve exibir a página de cadastro', async ({ page }) => {
    ...
    const heading = page.locator('h2:has-text("Crie sua conta")');
    await expect(heading).toBeVisible();
  ...
  test('deve mostrar erro ao tentar login com credenciais inválidas', async ({ page }) => {
    ...
    const errorMessage = page.locator('text=Email não cadastrado, Senha incorreta');
