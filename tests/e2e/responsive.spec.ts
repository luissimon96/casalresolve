...
test.describe('Responsividade', () => {
  test('deve exibir o menu mobile em telas pequenas', async ({ page }) => {
    ...
    // Verificar se o menu expandido está visível
    const expandedMenu = page.locator('a:has-text("Preços")');
  ...
  test('deve exibir o layout desktop em telas grandes', async ({ page }) => {
    ...
    const desktopNav = page.locator('nav >> a:has-text("Preços")');
    await expect(desktopNav).toBeVisible();
    
    // Verificar se o botão do menu mobile está oculto
    const menuButton = page.locator('button[aria-expanded="false"]');
